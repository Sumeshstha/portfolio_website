...
// Configure PDF.js worker from same origin to avoid CDN/ad-block/CSP issues
      ...
      <div className="bg-white dark:bg-dark-bg rounded-2xl shadow-2xl w-full h-full max-h-[95vh] flex flex-col transition-all duration-300 ease-in-out
        ...
        <div className="flex-grow overflow-auto p-2 sm:p-3 md:p-4 lg:p-6 transition-all duration-300 ease-in-out">
          <div className="pdf-viewer-container w-full h-full min-h-[50vh] sm:min-h-[60vh] md:min-h-[70vh] transition-all duration-300 ease-in-out">
            {useNativeViewer ? (
              <div className="w-full h-full min-h-[50vh] sm:min-h-[60vh] md:min-h-[70vh] bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                <iframe
                  title="Resume PDF"
                  src={`${resumeFile}#zoom=${Math.round(scale * 100)}`}
                  className="w-full h-full min-h-[50vh] sm:min-h-[60vh] md:min-h-[70vh]"
                />
              </div>
            ) : (
              <Document
                ...
                error={
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center p-4 text-gray-500 dark:text-gray-300">
                      <p className="text-sm sm:text-base font-semibold">Switching to browser PDF viewer...</p>
                    </div>
                  </div>
                }
